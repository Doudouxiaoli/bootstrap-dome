...
import com.kingyee.common.util.QRCodeUtil;
...
public class WxPortalController {
    ...
    public String getOpenId(String code, String url) {
        try {
            ...
            try {
                ...
                //                WdsWechatUser wdsWechatUser = wechatUserService.getWechatUserByOpenid(openId);
                //当前是否已关注
                if (wxMpUser.getSubscribe() != null && wxMpUser.getSubscribe()) {//已关注
                    //                    if (wdsWechatUser == null) {
    ...
    public String createTmpQrCode(String ticket) {
        try {
            ...
            WxMpQrCodeTicket qrCodeTicket = wxService.getQrcodeService().qrCodeCreateTmpTicket(ticket, 2592000);
     ...
     * @param doctorId
     */
    @RequestMapping(value = "createLastQrCode")
    @ResponseBody
    public String createLastQrCode(Integer doctorId) {
        try {
            WxMpQrCodeTicket qrCodeTicket = wxService.getQrcodeService().qrCodeCreateLastTicket(doctorId);
            QRCodeUtil.createQRImage(qrCodeTicket.getUrl(), "D:/medhos/doctor/", doctorId + ".jpg");
        } catch (Exception e) {
            e.printStackTrace();
