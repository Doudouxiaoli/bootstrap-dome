...
public class ExcelUtils {
    ...
    public static void exportExcel(File file, ExcelData data, Integer rowIndex, OutputStream out) throws Exception {
        SXSSFWorkbook wb = new SXSSFWorkbook(new XSSFWorkbook(new FileInputStream(file)), 5000);
        try {
            String sheetName = data.getName();
            if (null == sheetName) {
                sheetName = "Sheet1";
     ...
     * excel含有多个标签页
     */
    public static void exportSheetExcel(HttpServletResponse response, String fileName, List<ExcelData> list, boolean index) throws Exception {
        // 告诉浏览器用什么软件可以打开此文件
        response.setHeader("content-Type", "application/vnd.ms-excel");
        // 下载文件的默认名称
        response.setHeader("Content-Disposition", "attachment;filename=" + URLEncoder.encode(fileName, "utf-8"));
        exportSheetExcel(list, response.getOutputStream(), index);
    }
     ...
     * 大数据导出
     */
    public static Workbook exportSheetExcel(List<ExcelData> list, OutputStream out, boolean index) throws Exception {
        SXSSFWorkbook wb = new SXSSFWorkbook(5000);
        try {
            SXSSFSheet sheetIndex = null;
            CreationHelper createHelper = null;
            CellStyle hlink_style = null;
            if (index) {
                sheetIndex = wb.createSheet(WorkbookUtil.createSafeSheetName("索引"));
                createHelper = sheetIndex.getWorkbook().getCreationHelper();
                hlink_style = sheetIndex.getWorkbook().createCellStyle();
                Font hlink_font = sheetIndex.getWorkbook().createFont();
                hlink_font.setUnderline(Font.U_SINGLE);
                hlink_font.setColor(IndexedColors.BLUE1.getIndex());
            ...
            for (int i = 0; i < list.size(); i++) {
                ...
                if (index) {
                    Row indexRow = sheetIndex.createRow(i);
                    indexRow.setHeightInPoints(14);
                    Cell cell = indexRow.createCell(0);
                    cell.setCellValue((i + 1) + "." + sheetName);
                    Hyperlink link2 = createHelper.createHyperlink(HyperlinkType.DOCUMENT);
                    link2.setAddress("'" + WorkbookUtil.createSafeSheetName(sheetName) + "'!A1");
    ...
    private static void writeExcel(Workbook wb, SXSSFSheet sheet, ExcelData data) {
        int rowIndex = 0;
        if (data != null) {
    ...
    public static void writeExcel(Workbook wb, SXSSFSheet sheet, ExcelData data, Integer rowIndex) {
        if (rowIndex == null) {
            rowIndex = sheet.getLastRowNum() + 1;
        }
        if (data != null) {
    ...
    public static int writeRowsToExcel(Workbook wb, Sheet sheet, List<List<Object>> rows, int rowIndex) {
        int colIndex = 0;
    ...
    public static int writeRowsToExcel(Workbook wb, Sheet sheet, List<List<Object>> rows, int rowIndex, List<Integer> num) {
        int colIndex = 0;
     ...
     * @return
     */
    public static int writeRowsToExcel(Workbook wb, Sheet sheet, List<List<Object>> rows, int rowIndex, int colIndex) {
        Font dataFont = wb.createFont();
        //dataFont.setFontName("simsun");
        dataFont.setColor(IndexedColors.BLACK.getIndex());
    ...
    public static void autoSizeColumns(SXSSFSheet sheet, int columnNumber) {
